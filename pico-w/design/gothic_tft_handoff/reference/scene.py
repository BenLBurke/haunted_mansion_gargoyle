...
class Scene:
    ...
    def draw_facade(self):
        ...
        d.fill_rectangle(50, _fb(42, 42), 222, 42, body)
        ...
        d.fill_rectangle(108, _fb(54, 16), 7, 16, sil)       # chimney
        d.fill_rectangle(106, _fb(56, 3), 11, 3, sil)        # cap

        # flanking pitched towers: pentagon, peak at 50%, shoulders at 32%
        for x in (46, W - 44 - 36):
            self._pentagon(x, 36, 64, sil)

        # left tall tower + spire
        d.fill_rectangle(6, _fb(56, 56), 44, 56, sil)
        self.fill_tri(1, _fb(98), 28, _fb(98) - 44, 54, _fb(98), sil)
        d.fill_rectangle(27, _fb(106, 10), 2, 10, sil)
        self._diamond(28, _fb(111, 7) + 3, 4, sil)

        # right tower + spire
        d.fill_rectangle(W - 8 - 40, _fb(50, 50), 40, 50, sil)
        rx = W - 3 - 50
        self.fill_tri(rx, _fb(86), rx + 25, _fb(86) - 38, rx + 49, _fb(86), sil)
        d.fill_rectangle(W - 27 - 2, _fb(93, 9), 2, 9, sil)
        self._diamond(W - 27, _fb(97, 6) + 3, 3, sil)

        # window glow halo (two dim rings), then the lit pair
        d.fill_circle(28, _fb(31), 13, self.rgb(GLOW_RING_2))
        d.fill_circle(28, _fb(31), 8, self.rgb(GLOW_RING_1))
        self.fill_arch_gradient(18, _fb(41, 17), 8, 17, WIN_LIT)
        self.fill_arch_gradient(30, _fb(41, 17), 8, 17, WIN_DIM1)
        self.fill_arch_gradient(W - 20 - 8, _fb(36, 16), 8, 16, WIN_DIM2)
        for x in (88, 226):
            self.fill_arch(x, _fb(27, 15), 7, 15, self.rgb(WINDOW_DARK))

        # rose window
        d.fill_circle(161, _fb(38), 7, self.rgb(ROSE_EDGE))
        d.fill_circle(161, _fb(38), 4, self.rgb(ROSE_CORE))

        # entry arch + inner glow
        self.fill_arch(151, _fb(26, 26), 20, 26, sil, shoulder=0.36)
        self.fill_arch_gradient(154, _fb(20, 20), 14, 20,
                                ((145, 101, 45), (110, 74, 32), (74, 48, 22)),
                                shoulder=0.36)

        # ground fade
        self.v_gradient(0, _fb(20, 20), W, 20,
    ...
    def draw_chrome(self):
        ...
        self.d.draw_text8x8(88, 9, "HAUNTED MANSION", self.rgb(TITLE))
